...
file. In this section, we will see how we can combine these panels into
complete figures, which can be directly inserted into your manuscript.
...
Let's look at an example. We start by creating three plots, using the
...
Next, we combine them into a single figure using `patchwork`. 
  ...
  + The `|`symbol is used when we want panels to appear next to each other, in the
same row. 
  + The `/` symbol is used when we want panels to appear
underneath each other, in the same column. 

`patchwork` will do its best to align the axes of the different panels as well as possible, to
provide a nice and clean visualization.
...
`patchwork` works with base plots as well as plots generated using
`ggplot2`.
...
:::{.callout-warning title="exercise " icon="false"}
Add a plot of the sepal length vs the sepal width per one species, with on the side the image of that species.<br>

<details>
<summary><b>
Possible solution - try it by yourself first
</b></summary>
 
 + Look for the image of the species on internet. 
 + in the same folder as `qmd-example.qmd` create a subfolder called 'figs' to store the images
 + save the images in the 'figs/' folder with the corresponding name, e.g. "virginica.jpeg". <br>
 Save them with a name that has the same structure, e.g. "<species>.png" or "<species>.jpeg" if you want to use the parameters.
 + in the `qmd-example.qmd` add

```{{r}}
library(ggplot2)
library(patchwork)

p1 <- ggplot(species_oi, aes(x = Sepal.Length, y = Sepal.Width)) + 
    geom_point(size = 3) + 
    theme_bw()
picture <- cowplot::ggdraw() + cowplot::draw_image(paste("figs/",params$species,".jpeg", sep=""))

p <- (p1 | picture)
p + plot_annotation(
    title = paste("Species", params$species),
    tag_levels = 'A'
) & 
    theme_minimal()
```
