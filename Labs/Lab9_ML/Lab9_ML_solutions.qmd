...
## Question: Which of the two variables do you expect will have more predictive power?
glutest
:::
...
::: {.callout-note collapse="true"}
## Question: How did the prediction error change compared to the model with only 2 variables?
It decreased - the prediction error is almost half compared to what we got with only the 
insulin and glutest variables.
:::

Is the classifier based on 5 variables better, or are we overfitting our 
dataset? Cross-validation  in the next section will provide one possible answer 
to this question.  
...
```{r}
#| label: estimate_mcl_loocv
#| eval: !expr (! isTRUE(params$skip_execution))

#' A function to perform LOOCV
#' 
#' @param x matrix of predictor variables (rows are observations, columns are variables)
#' @param resp factor, response variable (length should be the same as nrow(x))
#' 
#' @value tibble with two columns, containing mean training and test error
#' 
...
# we will repeat the whole process 10 times and get average training and CV error
mcl = replicate(10, {
  # generate matrix of random values - columns are different variables
  xmat = matrix(rnorm(n * last(p)), nrow = n)
  # generate vector for response variable
  resp = sample(c("apple", "orange"), n, replace = TRUE)
  # add 1 and 2  for apples and oranges, respectively, to first six columns
  # in this way, these 6 variables become correlated with the response variable
  xmat[, 1:6] = xmat[, 1:6] + as.integer(factor(resp))
  # perform LOOCV with successively more variables included in the model
  lapply(p, function(k) {
...
ggplot(mcl, aes(x = p, y = value, col = variable)) + geom_line() +
  ...
  labs(
    ...
    x = "Number of successive variables in the model"
  )
```
...
If you want, you can explore `eh` object to see what other datasets there are.

For the following exercises, let's focus on the normal (`n`) and `cancer` 
samples, and set the adenomas aside.
...
::: {.callout-note collapse="true"}
## Question: What is the true positive rate (TPR, i.e. sensitivity) and true negative rate (TNR, i.e. specificity)?
Hint: See book or [Wikipedia](https://en.wikipedia.org/wiki/Sensitivity_and_specificity).
...
::: {.callout-note collapse="true"}
## Question: For how many different values of $\lambda$ did `glmnet` fit the model using settings as above?
...
::: {.callout-note collapse="true"}
## Question: For each of the values of $\lambda$ in the object above, calculate the L1 Norm (as we did above for two values of $\lambda$). Which $\lambda$ most closely corresponds to a L1 norm 6000?

```{r}
#| label: lambda_6k
#| eval: !expr (! isTRUE(params$skip_execution))

idx_L1Norm_6k <- which.min( abs( colSums(abs(glmfit$beta)) - 6000))    
...
::: {.callout-note collapse="true"}
## Question: How many non-zero coefficients do you get for the $\lambda$ that you found in the previous question?

```{r}
#| label: lambda_6k_nonzero
#| eval: !expr (! isTRUE(params$skip_execution))

sum(abs(coef(glmfit, s=lambda_6k)) > 0)
```
:::
...
::: {.callout-note collapse="true"}
## Question: How does the confusion table look like for $\lambda$ = `lambda.1se`? Report the top left element of the confusion table (i.e. number of correctly classified cancer samples). 
...
preds_1se <- predict(glmfit, newx = t(exprs(zellerNC)), s=s0, type = "class")
...
::: {.callout-note collapse="true"}
## Question: What features drive the classification (at $\lambda$ = `lambda.1se` chosen by cross-validation with the 1 standard error rule)? Report the top one (the one with the largest absolute value of its coefficient).


```{r}
#| label: idx_largest_beta
#| eval: !expr (! params$skip_execution & ! params$skip_answers)
idx_largest_beta <- which.max(abs(as.numeric(coef(glmfit, s=s0))))
...
::: {.callout-note collapse="true"}
## Question: How many methods does `caret` currently include?
