...
author: "Wolfgang Huber, Susan Holmes, TAs of the Stanford class 'Modern Statistics for Modern Biology', Julia Ruehle, Helena Crowell"
date: "7/10/2024"
format: 
  html:
    code-fold: false
...
Reproduce the below scatter plot (`geom_point()`) of gene expression `mean`s vs. `total` variance estimates. You will need a 2nd layer to highlight the top 2000 genes selected above (red points), and a 3rd layer to add the `tech`nical variance estimate (blue dashed line).
...
```{r mv-var, fig.width=4, fig.height=4}
#| eval: true
#| echo: !expr params$include_answers
...
By default, `runPCA()` computes the top 50 principal compared and stores them in resulting object in the `reducedDim()` slot:
...
Finally, note that the dataset retrieved from `ExperimentHub` already contains a non-linear embedding, t-SNE, that was computed by the original authors. We can access this as follows, and you are welcomed to further visually explore the data on your own. E.g., 
