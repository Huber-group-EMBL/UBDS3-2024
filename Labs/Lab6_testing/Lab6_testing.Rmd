...
(which is alphabetical) by one that makes more sense to us. This is needed to avoid warning messages when constructing the DEseq2 object.<br>

**Hands-on**: change `condition` to a factor with levels of all possible conditions; and `type` to a factor with levels of all possible type. Tip: you can use the `factor()` and `mutate()` functions

```{r factors,  results='hide'}
# TODO: use some code to solve this
```
...
We are going to use ``DESeq2``. To use the functions from this package, first we need to create a ``DESeqDataSet`` using:
  ...
  2. the ``pasillaSampleAnno`` sample annotation dataframe , where per each sample we have different information, here the sample id are stored in the `file` column. This would be the metadata file. 
...
**Hands-on:** take a look at the samples id in the two dataset (remember that in the `pasillaSampleAnno` they are in the `file` ), make them matching. 
Check that they are in the exact same order, that's critical for running DESeq2.
```{r solution hands-on}
# TODO: use some code to solve this
```
```{r hands-on, message = FALSE, warning = FALSE, results="hide", echo=F}
...
```{r DESeq2.2, include=TRUE, results='hide'}
...
```{r solution13-PCA, message=FALSE}
# TODO: use some code to solve this
```

```{r PCA, echo=F, message=FALSE, result = FALSE}
# TODO: use some code to solve this
...
With a PCA we can see that the `...`
...
**Hands-on:**<br>
Rerun the differential expression analysis, but this time account for `...` using a multi-factor design. For this, go to the [DESeq2 vignette](https://bioconductor.org/packages/devel/bioc/vignettes/DESeq2/inst/doc/DESeq2.html#multi-factor-designs) and read about how to use multi-factor designs.

```{r replaceDesign}
# TODO: use some code to solve this

```
```{r replaceDesign2, echo=F, message=F}
